...
use nixrs::daemon::{ClientOptions, DaemonError, DaemonResult, DaemonStore as _, LoggerResult, UnkeyedValidPathInfo};
use nixrs::daemon::server;
use nixrs::hash::{digest, Algorithm, Context, NarHash};
use nixrs::store_path::{StorePath, StorePathSet};
use nixrs::archive::{test_data, parse_nar, NAREvent};
      ...
      E: From<DaemonError>,
{
    ...
    try_join!(client, server, child.wait().map_err(|err| From::from(DaemonError::from(err)))).map(|_| ())
}
