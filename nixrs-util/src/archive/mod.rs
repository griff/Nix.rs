...
pub use dump::{dump,All, Filter, DumpOptions};
...
pub mod proptest {
        ...
        fn events(self, mut offset: u64, ls: &mut Vec<NAREvent>) -> u64 {
            match self {
                NarTree::Regular(executable, contents) => {
                    ...
                    if size > 0 {
                        let e = NAREvent::RegularNode { executable, size, offset };
                        offset += e.encoded_size() as u64;
                        let e = NAREvent::RegularNode { executable, size, offset };
                        ls.push(e);
                        let e = NAREvent::Contents { total: size, index: 0, buf: contents.into() };
                        offset += e.encoded_size() as u64;
                        ls.push(e)    
                    } else {
                        let e = NAREvent::RegularNode { executable, size, offset:0 };
                        offset += e.encoded_size() as u64;
                        ls.push(e);
                    }
                },
                NarTree::Symlink(target) => {
                    let e = NAREvent::SymlinkNode { target: Arc::new(target) };
                    offset += e.encoded_size() as u64;
                    ls.push(e);
                },
                NarTree::Dir(tree) => {
                    ...
                    for (name, node) in tree {
                        let e = NAREvent::DirectoryEntry { name: Arc::new(name) };
                        offset += e.encoded_size() as u64;
                        ls.push(e);
    ...
    pub fn arb_nar_events(
        ...
        arb_nar_tree(depth, desired_size, expected_branch_size)
            .prop_map(|tree| tree.to_events())
    }
