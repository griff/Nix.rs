...
pub trait AsyncSink {
    ...
    fn write_time(&mut self, time: SystemTime) -> Either<WriteU64<&mut Self>, Ready<io::Result<()>>>;
    ...
    fn write_printed<'this, 'item, 'async_trait, S, I>(&'this mut self, state: S, item: &'item I) -> Pin<Box<dyn Future<Output=io::Result<()>> + 'async_trait>>
        where 'this: 'async_trait,
              'item: 'async_trait,
            S: StatePrint<I> + 'async_trait,
            Self: Unpin;
    fn write_printed_coll<'this, 'async_trait, 'item, C, S, IT, I>(&'this mut self, state: S, coll: C) -> Pin<Box<dyn Future<Output=io::Result<()>> + 'async_trait>>
        where 'this: 'async_trait,
              'item: 'async_trait,
              S: StatePrint<I> + 'async_trait,
    ...
    */
    ...
    fn write_time(&mut self, time: SystemTime) -> Either<WriteU64<&mut Self>, Ready<io::Result<()>>> {
        match time.duration_since(SystemTime::UNIX_EPOCH) {
            ...
            Err(_) => Either::Right(future::err(io::Error::new(io::ErrorKind::InvalidInput, "time is before unix epoch")))
        }
    }
