...
impl<'r> ReadFrom<nix_daemon_capnp::store_path::Reader<'r>> for StorePath {
    fn read_from(value: nix_daemon_capnp::store_path::Reader<'r>) -> Result<Self, Error> {
...
impl<'b> BuildFrom<StorePath> for nix_daemon_capnp::store_path::Builder<'b> {
...
impl SetterInput<nix_daemon_capnp::store_path::Owned> for &'_ StorePath {
    fn set_pointer_builder(
        builder: capnp::private::layout::PointerBuilder<'_>,
        input: Self,
        _canonicalize: bool,
    ) -> capnp::Result<()> {
        let mut builder = nix_daemon_capnp::store_path::Builder::init_pointer(builder, 0);
...
impl<'b> BuildFrom<ContentAddress> for nix_daemon_capnp::content_address::Builder<'b> {
    fn build_from(&mut self, input: &ContentAddress) -> Result<(), Error> {
        match input {
            ...
            ContentAddress::Flat(hash) => {
                self.reborrow().set_flat(hash)?;
            }
            ContentAddress::Recursive(hash) => {
                self.reborrow().set_recursive(hash)?;
            }
        }
        Ok(())
    }
...
impl<'r> ReadFrom<nix_daemon_capnp::content_address::Reader<'r>> for ContentAddress {
    fn read_from(value: nix_daemon_capnp::content_address::Reader<'r>) -> Result<Self, Error> {
        match value.which()? {
            nix_daemon_capnp::content_address::Which::Text(hash) => {
                let digest = hash?;
                let hash = hash::Sha256::from_slice(digest)
                    .map_err(|err| Error::failed(err.to_string()))?;
                Ok(ContentAddress::Text(hash))
            }
            nix_daemon_capnp::content_address::Which::Flat(hash) => {
                let hash = hash?.read_into()?;
                Ok(ContentAddress::Flat(hash))
            }
            nix_daemon_capnp::content_address::Which::Recursive(hash) => {
                let hash = hash?.read_into()?;
